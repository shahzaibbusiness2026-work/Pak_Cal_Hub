    ...
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xs dark:border-slate-800 dark:bg-slate-900">
      ...
      <div className="divide-y divide-slate-100 dark:divide-slate-800">
        {rows.map((row, idx) => (
          ...
          >
            <div>
              <div className={`font-medium ${row.isTotal ? 'text-emerald-900 dark:text-emerald-300 font-bold' : 'text-slate-800 dark:text-slate-200'}`}>
                {row.label}
              </div>
              {row.detail && (
              ...
              <span
                className={`font-semibold ${
                  row.isDeduction
                    ...
                    : row.isTotal
                    ? 'text-emerald-900 dark:text-emerald-400 text-base'
